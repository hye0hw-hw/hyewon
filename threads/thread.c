...
/* Ready 큐 정렬 함수 (thread_unblock보다 앞에 위치) */
bool
...
/* 우선순위를 갱신 (Original vs Donated 중 Max) */
void
thread_update_priority (struct thread *t)
{
  ...
  if (!list_empty(&t->donations)) {
    ...
    donated_priority = highest_donor->priority;
  }
  
  t->priority = MAX(t->original_priority, donated_priority);
}

/* 락 해제 시 기부 리스트에서 제거 (PANIC 오류 해결 핵심) */
void 
...
thread_tick (void)
{
    ...
    {
        highest_ready_thread = list_entry(list_front(&ready_list), struct thread, elem);
        
        if (t->priority < highest_ready_thread->priority) {
            intr_yield_on_return ();
            return;
        }
    }

    /* Enforce Round-Robin preemption. */
...
{
    struct thread *t = NULL; // ⭐️ NULL 초기화 (경고 해결) ⭐️
